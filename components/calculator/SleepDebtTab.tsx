...
export default function SleepDebtTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid sm:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-medium text-mist mb-2">
            Hours slept last night: <span className="text-linen font-bold">{hoursSlept}h</span>
          </label>
          <input
            type="range"
            min={1}
      ...
      >
        {status === "debt" ? (
          <>
            ...
            <p className="font-serif text-4xl text-linen">{result.debt}h short</p>
          ...
          <>
            ...
            <p className="font-serif text-4xl text-linen">+{result.surplus}h over max</p>
            <p className="text-mist text-sm mt-2">
              {result.label}s need {result.recommended.min}–{result.recommended.max}h · you slept {hoursSlept}h
            </p>
          </>
          ...
          <>
            <p className="text-sm font-semibold text-ember mb-1">On Target</p>
            <p className="font-serif text-4xl text-linen">{hoursSlept}h</p>
            <p className="text-mist text-sm mt-2">
      ...
      <p className="text-xs text-mist/70">Source: CDC Sleep Recommendations 2023.</p>
    </div>
  );
}
